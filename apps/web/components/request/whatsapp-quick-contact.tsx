...
export function WhatsAppQuickContact() {
  ...
  const form = t.request.whatsappForm;
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");
  const [message, setMessage] = useState("");

  const businessDigits = useMemo(() => getPublicWhatsAppDigits(), []);

  const directHref = useMemo(
    () => buildWhatsAppQuickContactUrl(businessDigits, {}),
    [businessDigits],
  );

  function buildHref() {
    return buildWhatsAppQuickContactUrl(businessDigits, {
  ...
  function onSubmit(e: FormEvent) {
    e.preventDefault();
    openWhatsApp(buildHref());
  }

  return (
    <RequestFormShell>
      <div className="space-y-2 text-center">
        <div className="mx-auto flex size-16 items-center justify-center rounded-2xl bg-[#25D366]/15">
      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <div>
          <Label htmlFor="wa_phone">{form.phone}</Label>
          <IconInput
            id="wa_phone"
            value={phone}
        ...
        <div>
          <Label htmlFor="wa_message">{form.message}</Label>
          <IconTextarea
            id="wa_message"
            value={message}
        ...
        <button
          type="submit"
          className="inline-flex h-12 w-full items-center justify-center gap-2.5 rounded-[20px] bg-[#25D366] text-sm font-semibold text-white transition-opacity hover:opacity-90"
        >
